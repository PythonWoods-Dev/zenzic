...
- **Ecosystem Dependency Strategy & DQS Integrity Rules**: Codified governance rules (tracked internally) enforcing strict pinning (`==`) for `zenzic-action`, minimum versioning (`>=`) for `zenzic-vscode`, and 100% mathematical category-to-escalation parity in `scorer.py`.
