...
- **Sphinx adapter (GH #51) — retired 2026-09-15, planned for v0.32 until then.** Three reasons, each measured rather than assumed: Sphinx caches parsed sources as doctree pickles under `.doctrees`, and a Python pickle cannot be read without risking code execution, which the Zero Subprocess and No Inference invariants forbid; reStructuredText is not Markdown, so an adapter would have to add a parser rather than map files to routes, which is not what an adapter is; and Sphinx ships `linkcheck`, which already answers the central question for that ecosystem. What would reopen it: a Sphinx that emits a machine-readable route manifest, which the `prebuilt` engine already consumes. **And the exception, stated rather than implied: a Sphinx project using MyST-Parser writes Markdown, so the second reason does not apply to it** — the engine can already read those sources, and such a site is served today by `standalone` or by `prebuilt` with a manifest. Sphinx is not categorically out; reStructuredText is.
