...
declaration alone does not guarantee the fix is actually *reachable*: 3 of
the 6 fixable codes (Z515/Z517/Z520) had a working Mutation class the CLI's
``zenzic fix`` already used, but no corresponding branch in the LSP's
``textDocument/codeAction`` handler -- the editor's manual Quick Fix
silently offered nothing for them, undetected until an unrelated feature's
Phase 1 investigation happened to notice it.
...
all" question, the exact class of drift found here.
"""
...
def test_every_fixable_code_wired_into_lsp_code_action() -> None:
    ...
    assert not missing, (
        ...
        f"(This exact class of drift was found and fixed for Z515/Z517/Z520 previously.)"
    )
