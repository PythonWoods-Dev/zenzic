...
![A real zenzic check all run against docs/archive/old-migration-guide.md after a directory_policies exemption for Z402: the orphan-page finding is gone, while Z410 (isolated document) and Z101 (a genuine broken link) still fire on the same file, and the run exits 1 with a DQS Final Score of 80/100 (Gate Failed)](../../assets/images/blog/governance_exemption_demo.webp)

*This is the fifth article in Zenzic's Foundations series on practical adoption patterns. Part 1, [Snapshot Your Debt: Adopting Quality Gates Without Fixing Everything First](2026-08-30-snapshot-your-debt-baseline-quality-gates.md), covers gating on regressions against a frozen baseline — a mechanism built for debt a team intends to pay down over time. This article covers the opposite case: content that is never coming back into the navigation, on purpose, and needs a permanent exemption instead of a temporary one.*
