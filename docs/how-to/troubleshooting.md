...
```html
<a href="../sitemap.html" data-zenzic-ignore>Sitemap</a>
```

The attribute covers what that tag can produce in the HTML hygiene tier (`Z120`–`Z124`) and the relative-link tier (`Z102`, `Z104`, `Z105`). It does **not** cover `Z101`: a broken route is decided against the Virtual Site Map, not against the tag, and is governed in `.zenzic.toml`. Nor does it cover the security tier — a page cannot silence its own `Z202`, `Z203` or `Z205` by editing its own anchor.

An attribute that suppresses nothing is reported as [`Z603 DEAD_SUPPRESSION`](../rules/Z603.md), so adding one "just in case" is visible rather than free.
