...
'[text](https://example.com)'.
    ...
    1  │  # Bare URL Example
    2  │
    3  ❱  Visit https://example.com for more information. Wrapping raw URLs in …
       │  ^^^^^^^^^^^^^^^^^^^
    4  │  Markdown link syntax gives readers a descriptive label instead of a l…
    5  │  opaque string, and it also lets renderers apply consistent link styli…
