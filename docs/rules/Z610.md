---
title: "Z610 — Required Frontmatter Missing"
---
...
Unlike the link rules, `Z610` is a **file-level** finding anchored at the top of the
document, so the directive belongs **above the frontmatter block** — exactly as shown.
Placed at the end of a line further down it suppresses nothing and is itself reported
as [`Z603 DEAD_SUPPRESSION`](Z603.md). Verified by execution; the placement differs
from the link rules and the difference is real.
...
MDX files accept the JSX comment form as well, and the two behave identically:
