---
title: "Z614 — Unapproved Domain Reference"
---
...
- :material-alert-circle:{ .lg .middle style="color: var(--zz-error);" } **Severity: Error**
    ...
    Penalty: **5.0 points** | Category: **`brand`**
...
Check out [official docs](https://zenzic.dev/).
```
...
The directive goes **at the end of the line the link is on**. `Z614` anchors to the
link, so a directive on the line above suppresses nothing and is itself reported as
[`Z603 DEAD_SUPPRESSION`](Z603.md) — verified by execution against a configuration
that makes this rule fire.

```markdown title="Markdown"
[External Spec](https://unapproved.example.org/spec) <!-- zenzic:ignore:Z614 -->
```
